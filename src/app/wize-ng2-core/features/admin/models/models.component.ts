...
export class ModelsComponent implements OnInit {
  ...
  private isAuditsInstalled: boolean = false;
  ...
  ngOnInit() {
    this.isAuditsInstalled = !!this.uiConfigService.getModule('@wizeapps/sequelize-audit');
  ...
  getScopedAndVersionedModels() {
    this.adminService.getScopedAndVersionedModels(this.isAuditsInstalled).subscribe(data => {
      this.scopeModels = data[0].rows;
      this.versionedModels = this.isAuditsInstalled ? data[1].rows: [];
      this.getModels();
    });
  }

  isPrivate(modelName) {
    const isModel = this.scopeModels.filter((p) => {
  ...
  isVersioned(modelName) {
    const isModel = this.versionedModels.filter((p) => {
      return p.modelName === modelName
    });
    return !!(isModel && isModel.length > 0);
  }
