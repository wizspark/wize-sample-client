...
export class RowDetailComponent implements OnInit {
  ...
  @ViewChildren(RelationshipComponent) relationViews:QueryList<RelationshipComponent>;
  ...
  runRules(entityName) {
    const view = this.relationViews.find((view) => view.relationship.target === entityName);
    this.dataTableService.showRuleModal({
      primaryEntity: this.entity,
      entity: this.getRelationshipOptions(entityName),
      edit: false,
      ...
      row: {},
      routes: this.routes,
      executionData: this.formatExecutionData(view.tableView.rows),
      customFormData: {
        attributes: this.getRelationshipOptions(entityName).factSchema.attributes,
        settings: {}
      }
    });
  }
