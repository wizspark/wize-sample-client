...
@Component({
  selector: 'dt-detail',
  templateUrl: './dt-detail.html',
  styleUrls: ['./dt-detail.scss'],
  providers: []
})

export class DetailDataTableComponent implements OnChanges, OnDestroy {
  ...
  getModelData(event) {
    ...
    if (this.primaryEntity.name !== this.dataTableInputConfig.entityName) {
      ...
      this.dataTableService.getRows(path, event.query, this.entity.apis.association, event.first, event.rows, event.sortField, event.sortOrder).subscribe((data) => {
        ...
        if (this.dataTableInputConfig.isAssociation) {
          ...
          if (!this.associatedRows) {
            this.dataTableService.getRows(associatedPath, event.query, this.entity.apis.association, event.first, event.rows, event.sortField, event.sortOrder).subscribe((data) => {
